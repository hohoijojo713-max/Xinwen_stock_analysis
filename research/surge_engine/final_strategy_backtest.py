...
for f in sorted(DATA.glob('kline_*.parquet')):
    ...
    roll20=g.close.transform(lambda s:s.rolling(20).max()); x['dd20']=x.close/roll20-1
...
for code,gp in df.groupby('code',sort=False):
    ...
    for i in range(60,len(gp)-21):
        ...
        future=gp.iloc[i+1:i+21]; feat=gp.iloc[i]
        vals={c:(float(feat[c]) if np.isfinite(feat[c]) else np.nan) for c in features}
        if any(pd.isna(v) for v in vals.values()): continue
...
train=res[res.sample=='train'].copy(); valid=res[res.sample=='valid'].copy()
...
for f in features:
    ...
    train['_bin']=pd.cut(train[f],bins=qs,include_lowest=True,duplicates='drop')
    g=train.assign(good=(train.future_mfe>=0.03)&(train.future_mae>-0.05)).groupby('_bin',observed=True).agg(n=('good','size'),good=('good','mean'),prime=('good','mean')).reset_index()
    g['utility']=g.good - 0.75/np.sqrt(g.n)
    best=g.sort_values(['utility','n'],ascending=[False,False]).iloc[0]
    single.append((f,float(best['_bin'].left),float(best['_bin'].right),float(best.good),int(best.n)))
candidates=[]
for a,b in itertools.combinations(single,2):
    f1,lo1,hi1,_,_=a; f2,lo2,hi2,_,_=b
    tr=train[train[f1].between(lo1,hi1)&train[f2].between(lo2,hi2)]
    if len(tr)<500: continue
    good=(tr.future_mfe>=0.03)&(tr.future_mae>-0.05)
    score=float(good.mean())+0.25*float((tr.future_mfe>=0.05).mean())+0.10*np.log1p(len(tr))/10
    candidates.append({'factors':[f1,f2],'ranges':{f1:[lo1,hi1],f2:[lo2,hi2]},'train_n':len(tr),'train_good':float(good.mean()),'train_mfe3':float((tr.future_mfe>=0.03).mean()),'train_mfe5':float((tr.future_mfe>=0.05).mean()),'train_mfe8':float((tr.future_mfe>=0.08).mean()),'train_fail5':float((tr.future_mae<=-0.05).mean()),'score':score})
selected=sorted(candidates,key=lambda z:(z['score'],z['train_n']),reverse=True)[:10]

def simulate_rule(signals, trail_pct):
    trades=[]
    sig_codes=set(signals.code.unique())
    for code in sig_codes:
        gp=df[df.code.eq(code)].reset_index(drop=True); idx={pd.Timestamp(d):i for i,d in enumerate(gp.date)}
        ss=signals[signals.code.eq(code)]
...
for rank,c in enumerate(selected,1):
    f1,f2=c['factors']; lo1,hi1=c['ranges'][f1]; lo2,hi2=c['ranges'][f2]
    train_sig=train[train[f1].between(lo1,hi1)&train[f2].between(lo2,hi2)]
    valid_sig=valid[valid[f1].between(lo1,hi1)&valid[f2].between(lo2,hi2)]
    for trail in [0.02,0.025,0.03]:
        for sample_name,sig in [('train',train_sig),('valid',valid_sig)]:
            tr=simulate_rule(sig,trail)
            if tr.empty: continue
            results.append({'rank':rank,'factors':[f1,f2],'ranges':c['ranges'],'trail_pct':trail,'sample':sample_name,'n':len(tr),'win_rate':float((tr.ret>0).mean()),'avg_ret':float(tr.ret.mean()),'median_ret':float(tr.ret.median()),'cum_compounded':float((1+tr.ret).prod()-1),'stop5_rate':float((tr.reason=='hard_stop').mean()),'protect_rate':float((tr.reason=='protect_1pct').mean()),'trail_rate':float((tr.reason=='trail').mean()),'t20_rate':float((tr.reason=='t20').mean()),'annual_trades_est':float(len(tr)/max(1,(tr.signal_date.max()-tr.signal_date.min()).days/365.25))})
out={'objective':'Final OOS strategy backtest. Screening fitted on 2022-2024 only; T+1 open entry; initial -5% stop; after +3% set +1% protection; after +5% trail high by configured percentage; conservative same-day stop precedence.','base_train_good':base_train,'base_valid_good':base_valid,'selected_train_rules':selected,'backtest':results}
(OUT/'final_strategy_summary.json').write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8'); print(json.dumps(out,ensure_ascii=False,indent=2))
